...
import com.api.v1.exceptions.users.DuplicatedPersonalInformationException;
    ...
    public Mono<Users> register(@Valid UserRegistrationRequestDto requestDto) {
        return userRepository
                ...
                .filter(e -> e.getSsn().equals(requestDto.ssn())
                        && e.getEmail().equals(requestDto.email())
                )
                .hasElements()
                .flatMap(exists -> {
                    if (exists) {
                        return Mono.error(new DuplicatedPersonalInformationException(requestDto.ssn(), requestDto.email()));
                    }
                    return Mono.defer(() -> {
